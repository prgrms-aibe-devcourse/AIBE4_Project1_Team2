...
const myreviewController = {
  ...
  updateReview: async (req, res) => {
    try {
      ...
      const { rate, title, content, img_path } = req.body;
      ...
      // 수정할 필드가 하나도 없는 경우
      if (rate === undefined && !title && !content && img_path === undefined) {
        return res.status(400).json({
          success: false,
          statusCode: 400,
      ...
      const result = await myreviewService.updateReview(reviewId, {
        rate,
        title,
        content,
        img_path,
